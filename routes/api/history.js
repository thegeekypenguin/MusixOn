...
//@acess private
router.post('/song',[auth,[
    body('title','title is required').not().isEmpty(),
    body('subtitle','subtitle is not required').not().isEmpty(),]
    ],async (req,res)=>{
        ...
        try{
            ...
            if(song){
                ...
                return res.status(400).json({errors: [{msg: 'Song already in the history'}]});
        ...
        // const {title,subtitle,img_url,download_url} = req.body;
        // // find the user and add the song in that
        // const newSong = {title,subtitle,img_url,download_url};
        // try{
        //     // addding the song
